...
create table agent_info (
    ...
    avatar      varchar(512)  null comment '头像：emoji 表情或图片 URL',
    model_id    bigint        null comment '关联模型 ID（model_config.id）',
    temperature double        not null default 1.0 comment '模型温度：0-2，值越高输出越随机',
    context_count int         not null default 5 comment '上下文数：每次对话携带的历史消息条数，1-20，超出的旧消息随窗口淘汰',
    top_p       double        null comment 'Top P：null 未启用（用模型默认），启用时范围 0.01-1.0',
    max_tokens  int           null comment '每次回复的最大 Token 数：null 未启用（用模型默认）',
    ...
    state_store varchar(32)   not null default 'memory' comment '会话状态存储：memory/jsonfile/redis/mysql',
